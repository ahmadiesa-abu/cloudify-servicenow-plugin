...
def create_ci(ctx, snow_host, snow_auth, **kwargs):
    table_name = ''
    ctx.logger.debug('node_type : {0}'.format(ctx.node.type))
    if ctx.node.type == 'cloudify.nodes.snow.LinuxServer':
        table_name = 'cmdb_ci_linux_server'
    elif ctx.node.type == 'cloudify.nodes.snow.UnixServer':
        table_name = 'cmdb_ci_unix_server'
    elif ctx.node.type == 'cloudify.nodes.snow.WindowsServer':
...
@with_auth
def delete_ci(ctx, snow_host, snow_auth, **kwargs):
    table_name = ''
    ctx.logger.debug('node_type : {0}'.format(ctx.node.type))
    if ctx.node.type == 'cloudify.nodes.snow.LinuxServer':
        table_name = 'cmdb_ci_linux_server'
